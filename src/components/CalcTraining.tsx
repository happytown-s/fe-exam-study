...
function loadStats(): Record<string, TopicStats> {
  try {
    const raw = localStorage.getItem('fe-calc-stats');
    if (raw) return JSON.parse(raw);
  } catch { /* ignore */ }
  const stats: Record<string, TopicStats> = {};
  for (const t of calcData.topics) {
...
function saveStats(stats: Record<string, TopicStats>) {
  localStorage.setItem('fe-calc-stats', JSON.stringify(stats));
}
...
const TOPICS = calcData.topics as Topic[];

export default function CalcTraining({ onBack }: { onBack: () => void }) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">Calculation Drills for FE Exam</p>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-gray-900 dark:text-white group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors">
                      {t.titleJa}
                    </h3>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {activeTab === 'test' && <TestTab questions={topic.questions.filter(q => q.type === 'test')} topicId={topic.id} onAnswer={recordAnswer} />}
...
function CheatSheetTab({ cheatsheet }: { cheatsheet: string }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
      ...
      >
        <span className="font-bold text-gray-900 dark:text-white">Step-by-step Guide</span>
...
/* ---- Example Tab ---- */
function ExampleTab({ example }: { example: { question: string; steps: string[]; answer: string } }) {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5">
      ...
      <div className="space-y-2 mb-4">
        {example.steps.map((step, i) => (
          ...
          >
            <span className="text-xs font-bold text-orange-500 bg-orange-100 dark:bg-orange-900/40 rounded px-2 py-1 h-fit whitespace-nowrap">
              Step {i + 1}
            </span>
      ...
      <button
        ...
        className="w-full py-2 rounded-lg bg-gradient-to-r from-red-500 to-orange-500 text-white font-medium hover:opacity-90 transition-opacity"
      >
        {revealedSteps < example.steps.length
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 text-center">
        ...
        <button
          ...
          className="px-6 py-2 rounded-lg bg-gradient-to-r from-red-500 to-orange-500 text-white font-medium hover:opacity-90 transition-opacity"
        >
          Retry
        </button>
      </div>
...
function TestTab({
  ...
  if (finished) {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 text-center">
        <div className="text-3xl font-bold bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent mb-2">
          Test Complete
        </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5">
      ...
      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mb-5">
        <div
          className="h-2 rounded-full bg-gradient-to-r from-red-500 to-orange-500 transition-all"
